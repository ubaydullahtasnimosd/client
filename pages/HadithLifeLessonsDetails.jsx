...
export const HadithLifeLessonsDetails = () => {
  const { id } = useParams();
  const [isModalOpen, setIsModalOpen] = useState(false);

  const { data: lesson, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["hadith-life-lesson", id],
    queryFn: async () => {
      const response = await axios.get(`${baseUrl}/hadith_life_lessons/${id}/`);
      return response.data;
    },
  });

  const { data: comments = [] } = useQuery({
    queryKey: ["comments", "hadith_life_lessons", id],
    queryFn: async () => {
      const response = await axios.get(
        `${baseUrl}/comment/content/hadith_life_lessons/${id}/comments/`
      );
      return response.data;
    },
  });
  ...
  return (
    <main className="min-h-screen bg-[#f5f5f5] dark:bg-[#0f1117] text-slate-900 dark:text-slate-50 overflow-x-hidden">
      <Title title={`${lesson?.hadithLessonName || "হাদিস থেকে জীবনের পাঠ"} — মাওলানা উবায়দুল্লাহ তাসনিম`} />

      {/* SECTION: DETAILS WITH UNIFIED SECTION SHELL */}
      <SectionShell id="hadith-lesson-detail">
        <CommentModal
          isModalOpen={isModalOpen}
          setIsModalOpen={setIsModalOpen}
          content_type="hadith_life_lessons"
          object_id={id}
        />
          ...
          <div className="mt-10 sm:mt-14">
            <div className="rounded-3xl border border-stone-200/80 bg-white p-6 sm:p-8 md:p-10 shadow-xs dark:border-stone-800 dark:bg-slate-950">
              ...
              <div className="mt-6 sm:mt-8">
                <CommentsList content_type="hadith_life_lessons" object_id={id} />
              </div>
            </div>
          </div>
        </div>
...
export default HadithLifeLessonsDetails;
