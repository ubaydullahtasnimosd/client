    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter(
        (b) =>
          ...
          b.bookPublication?.toLowerCase().includes(q),
      );
    }

    // Sort filter
    if (sortBy === "latest") {
      list.sort((a, b) => new Date(b.bookCreatedAt || 0) - new Date(a.bookCreatedAt || 0));
    } else if (sortBy === "oldest") {
      list.sort((a, b) => new Date(a.bookCreatedAt || 0) - new Date(b.bookCreatedAt || 0));
    } else if (sortBy === "title") {
      list.sort((a, b) => (a.bookTitle || "").localeCompare(b.bookTitle || "", "bn"));
    }

    return list;
  }, [books, searchQuery, sortBy]);

  // Paginated books for current page (9 items per page)
    ...
    <main className="min-h-screen bg-[#f5f5f5]  text-slate-900  overflow-x-hidden">
      ...
      <SectionShell id="books" className="bg-[#f5f5f5] ">
        <SectionHeader
          ...
          subtitle="লেখকের প্রকাশিত বইগুলোর সংক্ষিপ্ত পরিচিতি ও বিস্তারিত তথ্য এখানে পাবেন।"
          align="center"
        />
        ...
        <div className="mb-8 sm:mb-12 rounded-2xl border border-stone-200/80 bg-white p-4 sm:p-5 shadow-xs  ">
          ...
          <div className="mt-3 flex items-center justify-between text-xs text-stone-500  pt-3 border-t border-stone-100 ">
            <span>
              ...
              {books?.length && filteredBooks.length !== books.length ? ` • সর্বমোট ${books.length} টির মধ্যে` : ""}
            </span>
