...
export const LifeLessons = () => {
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get(`${baseUrl}/life_lessons/`);
  ...
  if (isLoading) {
    return (
      <main className="min-h-screen bg-slate-50/60 py-10 dark:bg-slate-950 md:py-14">
        <Title title="জীবন থেকে নেওয়া শিক্ষা" />
      ...
      <div className={container}>
        <header className="text-center">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-300 md:text-base">
            জীবনযাত্রার ঘটনা, উপলব্ধি ও অভিজ্ঞতা থেকে পাওয়া মূল্যবান শিক্ষাগুলো এখানে পড়ুন।
          </p>
        ...
        {!data?.length ? (
          <div className="mt-10">
            <EmptyState message="এখনও জীবন থেকে নেওয়া কোনো শিক্ষা প্রকাশ করা হয়নি।" />
          </div>
        ) : (
          <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {data.map((lesson) => (
              <article key={lesson.id} className={cardBase}>
                <Link
                  to={`/islam/life-lessons/${lesson.id}`}
                  ...
                  aria-label={`${lesson.lifeLessonName} পড়ুন`}
                >
                  <img
                    className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-[1.03]"
                    src={lesson.lifeLessonImg || "/logo2.jpg"}
                ...
                <div className="flex flex-1 flex-col p-5 md:p-6">
                  ...
                  <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                    ...
                    {new Date(lesson.lifeLessonCreateAt).toLocaleDateString(
                      "bn-BD"
                    )}
                  </p>
