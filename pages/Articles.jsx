...
export const Articles = () => {
  const { data, isLoading, isError } = useQuery({
    queryKey: ['articles'],
    queryFn: async () => {
      const response = await axios.get('https://server-iota-ebon-83.vercel.app/api/v1/articles_essays/');
          ...
          {data?.map((article) => (
            <div key={article.id} className="border border-gray-200 dark:border-slate-500 p-6 bg-white dark:bg-slate-600 shadow-md hover:shadow-xl transition-shadow duration-300 flex flex-col">
              ...
              <p className="text-gray-500 dark:text-gray-300 mb-4 text-sm">
                {article.articlesEssaysAuthor} • {new Date(article.articlesEssaysCreateAt).toLocaleDateString('bn-BD')}
              </p>
